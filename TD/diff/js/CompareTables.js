...
/*	
	...
	*/
	var stageNodes = $("#stage").children("*:visible");
	var prodNodes = $("#stage").children("*:visible");
	var maxNodes = (stageNodes.size() >=  prodNodes.size()  ? stageNodes.size()-1 : prodNodes.size()-1);
	for (var n=0; n<maxNodes; n++){
		if (! stageNodes[n].isEqualNode(prodNodes[n])) {
			stageNodes[n].style.background="yellow";
			prodNodes[n].style.background="yellow";
			console.log("node " + n +"diff stage/prod:" +$(stageNodes[n]).prop("outerHTML") +"/"+$(prodNodes[n]).prop("outerHTML"));
		}
	}
